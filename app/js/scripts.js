...
(function ($, window, document) {
  function ChooseSlides() {
    ...
    mySwiper[0].on('slideChangeTransitionEnd', function () {
      if ($('.js_product_card-product--body_slider').length) {
        ...
        var slider_big_index = $(".js_product_card-product--body_slider .swiper-slide-active").index() - 1;
  ...
  function triggerCountOfProduct() {
    $(".js_product_card-count--plus").on("click", function () {
      var input_val = +$(this).closest(".js_product_card-product--body_content__footer").find("input[type=number]").val();
      input_val++;
